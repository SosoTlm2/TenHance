...
.AUTHOR
Soso
This is a modified Winhance version for Windows 10
...
using namespace TenHanceExtensions
...
$SCRIPT:LogPath = "$env:TEMP\TenHance-Log_$(Get-Date -Format 'yyyyMMdd-HHmmss').log"
...
# Global paths
$SCRIPT:ConfigPath = Join-Path $env:ProgramFiles "TenHance\Config"
...
function Start-Log {
    ...
    @"
==== TenHance Log - $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') ====
...
function Backup-Registry {
    ...
    try {
        ...
        $backupDir = Join-Path $env:ProgramFiles "TenHance\Backups\REG"
        ...
        $isFirstRun = -not (Get-ChildItem -Path $backupDir -Filter "TenHanceBackup_FirstRun.reg" -ErrorAction SilentlyContinue)
        
        # Set backup filename based on whether it's first run
        if ($isFirstRun) {
            $backupFile = Join-Path -Path $backupDir -ChildPath "TenHanceBackup_FirstRun.reg"
        }
        else {
            $timestamp = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
            $backupFile = Join-Path -Path $backupDir -ChildPath "TenHanceBackup_$timestamp.reg"
        }
        ...
        if (-not $isFirstRun) {
            ...
            $backups = Get-ChildItem -Path $backupDir -Filter "TenHanceBackup_*.reg" | 
            Where-Object { $_.Name -ne "TenHanceBackup_FirstRun.reg" } |
...
# Function to install Microsoft Store and Remove TenHance Removal script and scheduled task
function Install-Store {
    try {
        # First, update BloatRemoval.ps1 to remove Store entry
        $bloatRemovalPath = "$env:ProgramFiles\TenHance\Scripts\BloatRemoval.ps1"
...
# Function to install Microsoft Edge and Remove TenHance Removal script and scheduled task
function Install-MicrosoftEdge {
    try {
        ...
        # Delete TenHance Edge tasks and scripts
        ...
        $scriptPath = "$env:ProgramFiles\TenHance\Scripts\EdgeRemoval.ps1"
...
# Function to install OneDrive and Remove TenHance Removal script and scheduled task
function Install-OneDrive {
    try {
        ...
        # Delete TenHance OneDrive tasks and scripts
        ...
        $scriptPath = "$env:ProgramFiles\TenHance\Scripts\OneDriveRemoval.ps1"
        if (Test-Path $scriptPath) {
            Remove-Item -Path $scriptPath -Force -ErrorAction SilentlyContinue
...
# Function to install Xbox and Remove TenHance Removal script and scheduled task
function Install-Xbox {
    try {
        $bloatRemovalPath = "$env:ProgramFiles\TenHance\Scripts\BloatRemoval.ps1"
...
function Initialize-RemoveAppsButton {
    ...
    $text.Text = "Check all apps you don't want on your PC. Windows might try to add them back later, but TenHance will keep removing them for you."
...
$EdgeRemovalScript = {
    ...
    # Source: TenHance (https://github.com/memstechtips/TenHance)"
...
function Remove-Edge {
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
        ...
        try {
            ...
            Register-ScheduledTask -TaskName "TenHance\EdgeRemoval" `
...
- Disable OneDrive features system-wide`n`nAre you sure you want to continue?" -Title "Warning" -Buttons "YesNo" -Icon "Warning"
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
        ...
        try {
            ...
            Register-ScheduledTask -TaskName "TenHance\OneDriveRemoval" `
                -Action $action `
                -Trigger $trigger `
                -User "SYSTEM" `
                -RunLevel Highest `
...
function Remove-SelectedBloatware {
    ...
    if ($bloatScriptContent.Count -gt 4) {
        ...
        $scriptPath = "$env:ProgramFiles\TenHance\Scripts\BloatRemoval.ps1"
        if (-not (Test-Path -Path (Split-Path -Parent $scriptPath))) {
            New-Item -ItemType Directory -Path (Split-Path -Parent $scriptPath) -Force
        }
        ...
        Register-ScheduledTask -TaskName "TenHance\BloatRemoval" `
            -Action (New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-File `"$scriptPath`"") `
            -Trigger (New-ScheduledTaskTrigger -AtStartup) `
            -User "SYSTEM" `
            -RunLevel Highest `
...
<LineBreak />- Disable web search in start menu
    ...
    Visibility="Collapsed">
            ...
            Text="&#xE946;" />
                ...
                FontSize="14"
                Foreground="DarkGray"
                Text="Learn more about TenHance" />
                    </StackPanel>
                </DockPanel>
                <!-- Main Content -->
    ...
    Foreground="{DynamicResource PrimaryTextColor}"
    Margin="10,0,0,10">
    <Run Text="TenHance is a PowerShell GUI application designed to optimize and customize Windows 10 and 11 systems." />
    <LineBreak />
    ...
    Foreground="{DynamicResource PrimaryTextColor}"
    Margin="10,10,0,10"
    Text="Create TenHance Desktop Shortcut" />
                            <Button
...
$CloseButton.Add_Click({
    ...
    if ($showDialog) {
        $heart = [char]0x2764
        $response = Show-MessageBox -Message "Thanks for using TenHance! $heart
...
Click 'Yes' to show your support!" `
            -Title "Support TenHance" `
            -Buttons "YesNo" `
            -Icon "Information"

        if ($response -eq 'Yes') {
            Start-Process "https://github.com/memstechtips/TenHance"
...
$CreateShortcutButton.Add_Click({
        ...
        $Shortcut = $WshShell.CreateShortcut("$env:USERPROFILE\Desktop\TenHance.lnk")
        $Shortcut.TargetPath = "powershell.exe"
        $Shortcut.Arguments = "-ExecutionPolicy Bypass -Command `"Start-Process powershell -ArgumentList '-NoProfile -ExecutionPolicy Bypass -Command `"`"irm \`"https://github.com/memstechtips/TenHance/raw/main/TenHance.ps1\`" | iex`"`"' -Verb RunAs`""
...
$agreement = Show-MessageBox -Message "Welcome to TenHance!

a modified Winhance mod
This tool enhances your Windows experience by modifying system settings.

By continuing, you agree that:

- You are using this software at your own risk, any lost data is your problem
- The author accept no liability for any issues that may occur

Press 'Yes' to agree and continue, or 'Cancel' to exit." `
    -Title "TenHance - Important Notice" `
    -Buttons "YESCancel" `
    -Icon "Warning"
...
# Initialize logging
Start-Log
Write-Log -Message "Script has been launched" -Severity 'INFO'
...
try {
    ...
    if ($backupPath) {
        Show-MessageBox -Message "Welcome to TenHance!
